...
import argparse
import os
from noise_removal import postprocess
class DocScanner(object):
    ...
    def scan(self, image_path):
        ...
        original = image.copy()
        # if image.shape[0]<=300 or im.shape<=300:
        #     image =  cv2.resize(image,(640,480))
        try:
            newdata=pytesseract.image_to_osd(image,config='--psm 0 -c min_characters_to_try=3')
            angle = re.search('(?<=Rotate: )\d+', newdata).group(0)
            angle =  int(angle)
        # print("angle",angle)
            if angle == 90 or angle == 180:
                # print("180 degree correction")
                image = cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE)
                
            if  angle==270:
                image = cv2.rotate(image, cv2.ROTATE_90_COUNTERCLOCKWISE)
                # print("270 degree correction")
        ...
        sharpen = postprocess(warped)
        if len(sharpen.shape)==3:
             h,w,c = sharpen.shape
        if  len(sharpen.shape)==2:
            h,w= sharpen.shape
            sharpen = cv2.cvtColor(sharpen,cv2.COLOR_GRAY2BGR)
      
        original = cv2.resize(original,(w,h))
       
        result =  cv2.hconcat([original,sharpen])
